...
- `FORM_URL` powers **Apply to Play**, **Join the Waitlist**, and footer apply links.
  - If `FORM_URL` is still `example.com`, those links fall back to `#apply` so visitors are still sent to the Apply section.
...
### 4) Update SEO URL placeholders

In `index.html`, update these tags before going live:

- `og:url` to your final GitHub Pages URL
- `og:image` / `twitter:image` to a real preview image path in `assets/img/`
